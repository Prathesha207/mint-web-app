...
export default function ROIToolsPalette({
  ...
  return (
    <div className="absolute left-4 top-1/2 -translate-y-1/2 z-31 flex flex-col gap-2 pointer-events-none">
      <div className="bg-[#16181D]/90 backdrop-blur-md border border-white/10 rounded-xl p-1.5 shadow-2xl flex flex-col gap-1 pointer-events-auto">
        <button
          ...
          className={`p-2.5 rounded-lg flex items-center justify-center transition-all ${drawingMode === 'select'
        ...
        <button
          ...
          className={`p-2.5 rounded-lg flex items-center justify-center transition-all ${drawingMode === 'rectangle'
            ? 'bg-blue-500/20 text-blue-400 shadow-inner'
            : 'text-neutral-400 hover:bg-white/10 hover:text-white'
            }`}
        ...
        <button
          ...
          className={`p-2.5 rounded-lg flex items-center justify-center transition-all ${drawingMode === 'polygon'
            ? 'bg-blue-500/20 text-blue-400 shadow-inner'
            : 'text-neutral-400 hover:bg-white/10 hover:text-white'
            }`}
          title="Polygon Tool"
        ...
        {drawingMode === 'polygon' && isDrawing && (polygonPointsLength ?? 0) >= 3 && (
          <button
            type="button"
            onClick={(e) => {
              e.stopPropagation();

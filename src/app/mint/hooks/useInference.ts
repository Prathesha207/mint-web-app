...
export function useInference({
  ...
  const cropProcessedFrameToRoi = useCallback(
    async (processedFrameBase64: string): Promise<string> => processedFrameBase64,
    []
  );
  ...
  const displayBufferedProcessedFrame = useCallback(
    (frame: BufferedProcessedFrame, options?: { followLive?: boolean }) => {
  ...
  const applyLatestProcessedFrame = useCallback(
    (
      ...
      setLiveProcessedFrame(bufferedFrame.processedFrame);
    ...
    [buildBufferedProcessedFrame, displayBufferedProcessedFrame, rememberBufferedProcessedFrame]
  );
  ...
  const fetchProcessedFrame = useCallback(
    async (
      jobId: string,
      frameNumber: number,
      options?: { updateDisplay?: boolean; requestId?: number }
    ) => {
      ...
      if (cachedFrame) {
        if (
          ...
          (typeof options?.requestId !== 'number' ||
            options.requestId === processedFrameViewRequestRef.current)
        ) {
  ...
  const getBufferedFrameNumbers = useCallback(() => {
    return Array.from(processedFrameBufferRef.current.keys()).sort((a, b) => a - b);
  }, []);

  const getAdjacentBufferedFrameNumber = useCallback(
    (currentFrame: number, direction: 'previous' | 'next') => {
      const frameNumbers = getBufferedFrameNumbers();
      if (frameNumbers.length === 0) return null;

      if (direction === 'previous') {
        for (let index = frameNumbers.length - 1; index >= 0; index -= 1) {
          if (frameNumbers[index] < currentFrame) {
            return frameNumbers[index];
          }
        }
        return frameNumbers[0] < currentFrame ? frameNumbers[0] : null;
      }
  ...
  const viewProcessedFrame = useCallback(
    async (jobId: string, frameNumber: number) => {
      ...
      setFollowLiveFrame(false);
      const success = await fetchProcessedFrame(jobId, frameNumber, {
        updateDisplay: true,
        requestId,
      });
      if (requestId !== processedFrameViewRequestRef.current) {
        return false;
      }
      if (!success) {
        return false;
  ...
  const viewAdjacentProcessedFrame = useCallback(
    async (jobId: string, currentFrame: number, direction: 'previous' | 'next') => {
      const bufferedFrameNumber = getAdjacentBufferedFrameNumber(currentFrame, direction);
      const fallbackFrameNumber =
        direction === 'previous' ? Math.max(0, currentFrame - 1) : currentFrame + 1;
      const targetFrameNumber = bufferedFrameNumber ?? fallbackFrameNumber;
      return viewProcessedFrame(jobId, targetFrameNumber);
    },
    [getAdjacentBufferedFrameNumber, viewProcessedFrame]
  );

  const viewPreviousProcessedFrame = useCallback(
    (jobId: string, currentFrame: number) =>
      viewAdjacentProcessedFrame(jobId, currentFrame, 'previous'),
    [viewAdjacentProcessedFrame]
  );

  const viewNextProcessedFrame = useCallback(
    (jobId: string, currentFrame: number) =>
      viewAdjacentProcessedFrame(jobId, currentFrame, 'next'),
    [viewAdjacentProcessedFrame]
  );
  ...
  const goLiveToLatestProcessedFrame = useCallback(
    async (jobIdOverride?: string) => {
      ...
      processedFrameViewRequestRef.current += 1;
      setFollowLiveFrame(true);
  ...
  return {
    ...
    fetchProcessedFrame,
    viewProcessedFrame,
    viewPreviousProcessedFrame,
    viewNextProcessedFrame,
